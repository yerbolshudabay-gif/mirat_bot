...
async def cmd_feedback(message: Message):
    ...
    await message.answer(
        ...
        "_Просто ответь на это сообщение текстом._",
        parse_mode="Markdown"
    )
...
@dp.message(F.text & F.text.startswith("/fb "))
async def handle_feedback_short(message: Message):
    text = message.text[4:].strip()
    if text:
        save_feedback(message.from_user.id, text)
...
async def cmd_admin(message: Message):
    if message.from_user.id != ADMIN_ID:
        return
    conn = sqlite3.connect("awareness.db")
    rows = conn.execute("SELECT timestamp, text FROM feedback ORDER BY timestamp DESC LIMIT 20").fetchall()
    conn.close()
    if not rows:
