...
# Глобальные состояния — объявлены ДО хэндлеров
WAITING_FEEDBACK: set = set()
...
def save_feedback(user_id, text):
    conn = sqlite3.connect("awareness.db")
    conn.execute("INSERT INTO feedback (user_id, timestamp, text) VALUES (?, ?, ?)",
...
async def cmd_feedback(message: Message):
    ...
    await message.answer(
        "💬 Напиши свой отзыв — что точно, что не точно, что улучшить.\n\n"
        "_Просто напиши текст следующим сообщением._",
        parse_mode="Markdown"
    )
...
async def cmd_admin(message: Message):
    ...
    conn = sqlite3.connect("awareness.db")
    rows = conn.execute(
        "SELECT timestamp, text FROM feedback ORDER BY timestamp DESC LIMIT 20"
    ).fetchall()
    conn.close()
    if not rows:
...
# F.text последним — и только не-команды
@dp.message(F.text & ~F.text.startswith("/"))
async def handle_text(message: Message):
    # Если ожидаем feedback — сохраняем без анализа
    if message.from_user.id in WAITING_FEEDBACK:
        WAITING_FEEDBACK.discard(message.from_user.id)
        save_feedback(message.from_user.id, message.text)
        await message.answer("✅ Спасибо — обратная связь получена.")
        return
...
async def main():
    init_db()
    logger.info("Bot started v10 — Мират стабильный")
